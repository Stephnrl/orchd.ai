...
authenticate the file against a trusted historical checkpoint.
...
Audits, staging and reservation check the exact supported SQLite table, automatic unique
indexes and three retention-trigger definitions inside their read or writer transaction.
The application identity/version must still match, check constraints must be enabled,
and temporary objects that could shadow the journal are rejected. Added tables/indexes,
missing triggers and same-name replacement triggers fail closed, including changes made
by another connection after opening the journal. Schema inspection is bounded to the
expected object count plus one and bounded SQL text.

This accepts the schema generated by the existing version-1 journal without migration or
changes to audit report digests. Manually recreated or extended schemas, even if intended
to be equivalent, are unsupported. Checks never repair or recreate missing protections.
Individual inspection and usage remain diagnostic reads and do not certify schema health;
use the whole-journal audit. Backup verification, comparison and drills inherit the audit
gate and reject structurally altered databases even with recomputed file checksums.

Definition checks do not prevent an administrator bypassing the application or editing
the file afterward, and do not prove that earlier evidence was never changed. Live
publishing and restore/adoption remain disabled.
