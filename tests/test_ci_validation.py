...
class CIValidationTests(unittest.TestCase):
    def result(self, ids, success=True, count=10):
        return SimpleNamespace(skipped=[(SimpleNamespace(id=lambda value=value: value), "reason") for value in ids],
                               testsRun=count, wasSuccessful=lambda: success, expectedFailures=[])
    ...
    def test_expected_failures_cannot_hide_regressions(self):
        result = self.result(['docker'])
        result.expectedFailures = [('hidden regression', 'trace')]
        self.assertFalse(accepted(result, ['docker']))
