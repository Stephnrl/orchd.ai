...
def capture(argv, *, cwd=None, env=None, timeout=20, limit=65536, input_bytes=None):
    ...
    if input_bytes is not None:
        # Inputs are small bounded envelopes; child consumes stdin before running tools.
        try:
    ...
    while process.poll() is None:
        if overflow.is_set() or time.monotonic() >= deadline:
            failure = "output_limit" if overflow.is_set() else "timeout"
            process.kill()
            break
        time.sleep(0.01)
