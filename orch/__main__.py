"""Every offline command this control plane offers, and nothing that decides anything.

One flat argument parser over every command, which is why the choices list is long. Each
command imports what it needs when it runs, so opening a store costs nothing for a command
that does not touch one, and no command here authorizes an effect that its module would not.
"""
import argparse
import json
...
def main():
    ...
    parser.add_argument("--jira-journal", help="Jira operation journal for status to report capacity from")
    ...
    parser.add_argument("--agent", help="Name of the agent holding an agent session, or the agent to enrol")
    ...
    if args.command in ('jira-approval-preview', 'jira-check-approval', 'jira-preflight-read-plan', 'jira-preflight'):
        ...
        if report.get('status') == 'blocked': raise SystemExit(2)
        return
    if args.command == 'task-issue-intent':
        if not (args.task and args.github_profile):
            parser.error('task-issue-intent requires --task and the --github-profile naming the destination')
        from .contracts import Rejected, canonical
        from .github_preview import load_intent
        from .task_issue import derive
        import sqlite3
        engine = None
        try:
            profile = load_intent(args.github_profile)
            engine = Engine(args.data, Executor('trusted-fixture' if args.trusted_fixture else 'docker', args.image))
            report = derive(engine, args.task, profile)
        except (Rejected, OSError, sqlite3.Error) as exc:
            parser.error('Cannot derive an issue intent: ' + str(exc))
        finally:
            if engine is not None: engine.close()
        # The intent alone, so it can be handed straight to the read plan that stages it.
        print(canonical(report['intent']).decode())
        return
    ...
    if args.command == 'skill-export':
        ...
        return
    if args.command == 'agent-enrol':
        from .agent_service import enrol
        from .contracts import Rejected, canonical
        if not (args.agents and args.agent and args.role):
            parser.error('agent-enrol requires --agents, --agent and one or more --role values')
        try:
            report = enrol(args.agents, args.agent, args.role.split(','))
        except (Rejected, OSError) as exc:
            parser.error('Cannot enrol that agent: ' + str(exc))
        # The path, never the secret: a secret that reaches a terminal reaches its history.
        print(canonical(report).decode())
        return
    ...
    if args.command in ("github-inspect", "github-journal-usage", "github-journal-audit"):
        ...
        try:
            ...
            if journal is not None:
                journal.close()
        return
    if args.command == "github-issue-stage":
        if not (args.intent and args.github_profile and args.transcript and args.expected_sha256 and args.journal):
            parser.error("github-issue-stage requires --intent, --github-profile, --transcript, "
                         "--expected-sha256 of the preview and --journal")
        from .github_preview import load_intent
        ...
        journal = None
        try:
            intent, profile = load_intent(args.intent), load_intent(args.github_profile)
            capture = load_intent(args.transcript)
            # Check the scope before opening anything, as github-stage does: a staging that
            # was never valid should not leave a journal database behind.
            issue_scope(intent, profile, capture, args.expected_sha256)
            journal = GitHubJournal(args.journal)
            print(json.dumps(journal.stage_issue(intent, profile, capture, args.expected_sha256), indent=2))
        except (Rejected, OSError, sqlite3.Error):
            parser.error("Cannot stage GitHub issue intent; check the preview digest, capture, "
                         "journal identity, capacity and availability")
        finally:
            if journal is not None:
                journal.close()
        return
    ...
    if args.command == "status":
        ...
        try:
            ...
            result = report(engine, args.journal, args.jira_journal, roster(args.agents))
        except (Rejected, OSError, sqlite3.Error):
            parser.error("Cannot report status; check the store path and any journals given")
        finally:
            if engine is not None: engine.close()
        print(canonical(result).decode())
        # Nothing here is a failure: attention means a person is needed, not that anything
        # is broken, and an operator scripting this should not have to parse an exit code
    ...
    try:
        if args.command == "serve":
            serve(engine, args.port, args.agents)
